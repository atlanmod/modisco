...
public class GenericXMLWriter extends PrintWriter {
	 ...
	 */
	public GenericXMLWriter(final OutputStream output)
			throws UnsupportedEncodingException {
			...
			if (root.getNamespaces() != null) {
				for (Namespace ns : root.getNamespaces()) {
					if (ns.getName().length() > 0) {
						printAttribute(
							GenericXMLHandler.XMLNS + ":" + ns.getName(), //$NON-NLS-1$
							ns.getValue());
					} else {
						printAttribute(
							GenericXMLHandler.XMLNS, ns.getValue());
					}
				}
			}
		}
	...
	private void printAttribute(final String key,
			final String value) {
		print(" "); //$NON-NLS-1$
		print(key);
		print("=\""); //$NON-NLS-1$
