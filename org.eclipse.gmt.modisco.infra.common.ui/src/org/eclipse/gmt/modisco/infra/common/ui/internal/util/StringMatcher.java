...
public class StringMatcher {
	 ...
	 */
	public StringMatcher.Position find(final String text, final int start, final int end) {
		int newStart = start;
		int newEnd = end;
	...
	public boolean match(final String text, final int start, final int end) {
		...
		if (this.fIgnoreWildCards) {
			return (newEnd - newStart == this.fLength)
					&& this.fPattern.regionMatches(this.fIgnoreCase, 0, text, newStart,
							this.fLength);
		}
		int segCount = this.fSegments.length;
	 ...
	 */
	protected boolean regExpRegionMatches(final String text, final int tStart, final String p,
			final int pStart, final int plen) {
