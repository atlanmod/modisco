...
public class TestEjbJar30 {
	...
	private static final String RESOURCES_TEST_XML = "/resources/ejb_jar_3_0.xml"; //$NON-NLS-1$
	...
	public void initResource() {
		if (this.resource == null) {
			...
			IPath path = new Path(wsLocation.replaceFirst("^reference:file:", "") //$NON-NLS-1$ //$NON-NLS-2$
					+ TestEjbJar30.RESOURCES_TEST_XML);
			...
			if (xmlFile.exists()) {
				...
				Assert.assertTrue(DeploymentDescriptorDiscoverer.getDescXmlVersion(
						...
						EjbJarDiscoverer.getDtdUrl()).equalsIgnoreCase(TestEjbJar30.VERSION));
