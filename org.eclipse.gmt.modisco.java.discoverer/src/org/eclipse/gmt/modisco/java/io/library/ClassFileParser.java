...
/**
 * The main class for populating the Java model from the JDT Java model
...
public class ClassFileParser {
	...
	/**
	 * the {@code ClassFile} object associated with the .class file of the top-level type
	 */
	...
	/**
	 * the currently visited java type or method.
	 * Useful to retrieve type variable definition.
	 */
	private IMember currentlyVisitedJavaElement = null;

	public ClassFileParser(final JavaFactory factory, final Model resultModel,
			final BindingManager globalBindings, final TypeFinder typeFinder,
	...
	/**
	 * Finds the type variable associated with the currently visited Java element (type or method)
	 * or with it's parent.
	...
	/**
	 * @see org.eclipse.gmt.modisco.java.io.java.JDTVisitor#initializeNode(ASTNode, org.eclipse.jdt.core.dom.ASTNode)
	 */
	 ...
	 * @param type the java model {@code IType}
	 * @param enclosingType the MoDisco enclosing type of the {@code IType}
	 * @return the {@link AbstractTypeDeclaration} object corresponding to the {@code IType}, {@code null}
	 * 		if type does not exist.
	 * @throws JavaModelException
	 */
	protected AbstractTypeDeclaration parse2(final IType type,
			final AbstractTypeDeclaration enclosingType)
			throws JavaModelException {
	 ...
	 * @param packageFragment the {@code IPackageFragment}
	 */
	protected void visitPackage(final IPackageFragment packageFragment) {
		...
		if (!packageFragment.exists() || packageFragment.getElementName().length() == 0) {
			...
			//named package
			Binding id = JavaModelDelegateBindingFactory.getInstance().getBindingForElement(packageFragment, this);
	...
	protected Package createPackageHierarchy(
	 ...
	 * @param type the {@code IType}
	 * @return the {@link AbstractTypeDeclaration} object corresponding to the {@code IType}
	 * @throws JavaModelException
	 */
	protected AbstractTypeDeclaration visitType(final IType type)
			throws JavaModelException {
	...
	/**
	 * Visit a {@code IField}.
	 * @param field the {@code IField}
	 * @return the {@link FieldDeclaration} object corresponding to the {@code IField}, or 
	 * 			{@code null} if the field does not exist or 
	 * 			is {@link org.eclipse.jdt.core.Flags#isSynthetic(int) synthetic}
	 * @throws JavaModelException
	 */
	protected FieldDeclaration visitField(final IField field)
			throws JavaModelException {
	...
	protected EnumConstantDeclaration visitEnumConstantDeclaration(
	 ...
	 * @param method the {@code IMethod}
	 * @return the {@link AbstractMethodDeclaration} object corresponding to the {@code IMethod}, or 
	 * 		{@code null} if the method does not exist or is {@link org.eclipse.jdt.core.Flags#isSynthetic(int) synthetic}
	 * 		or is {@link org.eclipse.jdt.core.Flags#isBridge(int) bridge} 
	 * 		or its name equals the special name <code>"&lt;clinit&gt;"</code>
	 * @throws JavaModelException
	 * @see org.eclipse.jdt.core.IMethod#getElementName()
	 */
	protected AbstractMethodDeclaration visitMethod(final IMethod method)
			throws JavaModelException {
	...
	protected void visitAnnotation(final IAnnotation annotation,
			final Annotation anno) throws JavaModelException {
		initializeNode(anno);
	...
	private Expression manageMemberValuePair(final IMemberValuePair defaultValue) {
		...
		case IMemberValuePair.K_SIMPLE_NAME:   //there should be no K_SIMPLE_NAME in .class files
	...
	private Expression manageValuePairQualifiedNameKind(
			final IMemberValuePair defaultValue) {
		Expression result;
		if (defaultValue.getValue().getClass().isArray()) {
			Object[] tab = (Object[]) defaultValue.getValue();
			ArrayInitializer array = getFactory().createArrayInitializer();
			...
			for (Object element : tab) {
				...
				Binding id = null;
				if (this.typeFinder.isTypeExists(typeName)) {
					ClassBinding parent = JavaModelDelegateBindingFactory.getInstance().getBindingForName(Signature.createTypeSignature(typeName, true), this, true);
					id = new FieldBinding();
			...
			//can't use JavaModelDelegateBindingFactory.getBindingForName(String, ClassFileParser)
	...
	private Expression manageValuePairAnnotationKind(
			final IMemberValuePair defaultValue) {
		Expression result;
		if (defaultValue.getValue().getClass().isArray()) {
			Object[] tab = (Object[]) defaultValue.getValue();
	...
	private Expression manageValuePairClassKind(
			final IMemberValuePair defaultValue) {
		Expression result;
		if (defaultValue.getValue().getClass().isArray()) {
			Object[] tab = (Object[]) defaultValue.getValue();
	 ...
	 * <p>
	 * Decodes the signature and tries to resolve immediately the reference. 
	 * If unresolvable, a pending reference is created.
	 * </p>
	 * So, the returned {@code TypeAccess} may or may not have a resolved {@link TypeAccess#getType() type}.
	 * @param qualifiedName the signature of the referenced type (eg : [I, Ljava.lang.Object;)
	...
	protected void visitParameterizedType(final TypeAccess type,
			final String qualifiedName) {
		Binding id = JavaModelDelegateBindingFactory.getInstance().getBindingForParameterizedType(qualifiedName, this, false);
	...
	protected void visitWildCardType(final TypeAccess type,
			final String qualifiedName) {
		Binding id = JavaModelDelegateBindingFactory.getInstance().getBindingForWildCardType(qualifiedName, this);
		...
		if (wildCardType == null) {
			...
			switch(qualifiedName.charAt(0)) {
				...
				case Signature.C_SUPER :
					boundSignature = qualifiedName.substring(1); //remove the first character
					break;
	...
	/**
	 * @see JDTVisitorUtils#manageBindingRef(org.eclipse.jdt.core.dom.PrimitiveType, JDTVisitor)
	 */
	protected void visitPrimitiveType(final TypeAccess type,
			final String qualifiedName) {
		Binding id = JavaModelDelegateBindingFactory.getInstance().getBindingForPrimitiveType(qualifiedName, this);
	...
	/**
	 * @see JDTVisitorUtils#manageBindingRef(org.eclipse.jdt.core.dom.ArrayType, JDTVisitor)
	 */
	protected void visitArrayType(final TypeAccess typAcc,
			final String qualifiedName, final ClassFileParser visitor) {
		Binding id = JavaModelDelegateBindingFactory.getInstance().getBindingForArrayType(qualifiedName, visitor, false);
	 ...
	 * @param modiscoModifier the MoDisco Modifier
	 * @see Flags
	 */
	private void manageModifier(final Modifier modiscoModifier,
			final int flags, final IJavaElement element) {
		...
		//visibility modifiers are applicable to types, methods, constructors, and fields.
		if (kind == IJavaElement.TYPE || kind == IJavaElement.METHOD || kind == IJavaElement.FIELD) {
