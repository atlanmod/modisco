...
public final class JDTDelegateBindingFactory implements IBindingFactory {
	...
	private PackageBinding getPackageBinding(final IPackageBinding binding) {
	...
	private VariableBinding getVariableBinding(final IVariableBinding binding) {
	...
	private boolean isLocalVariable(final Name name) {
	...
	private boolean isLocalMethod(final Name name) {
		boolean result = false;
		IBinding binding = name.resolveBinding();
		if (binding != null) {
			if (binding instanceof IMethodBinding) {
