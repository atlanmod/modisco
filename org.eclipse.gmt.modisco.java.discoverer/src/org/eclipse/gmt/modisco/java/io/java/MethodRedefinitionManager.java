...
public final class MethodRedefinitionManager {
	 ...
	 * @param facto
	 */
	public static void resolveMethodRedefinitions(final Model model,
			final JavaFactory facto) {
		Object item;
		for (Iterator<?> i = model.eAllContents(); i.hasNext();) {
			...
			if (item instanceof MethodDeclaration) {
				...
				if (!aMethod.isProxy()) {
					...
					AbstractTypeDeclaration declaringType = aMethod
							.getAbstractTypeDeclaration();
					if (declaringType != null
							&& declaringType instanceof ClassDeclaration) {
						superClassRef = ((ClassDeclaration) declaringType)
								.getSuperClass();
					}
					if (declaringType == null
							&& aMethod.getAnonymousClassDeclarationOwner() != null) {
						ClassInstanceCreation cic = aMethod
					...
					while (!bFound
						...
						ClassDeclaration superClass = (ClassDeclaration) superClassRef
								.getType();
						for (BodyDeclaration bodyDecl : superClass
								.getBodyDeclarations()) {
							if (bodyDecl instanceof MethodDeclaration
								...
								bFound = true;
								aMethod
										.setRedefinedMethodDeclaration((MethodDeclaration) bodyDecl);
							}
						}
