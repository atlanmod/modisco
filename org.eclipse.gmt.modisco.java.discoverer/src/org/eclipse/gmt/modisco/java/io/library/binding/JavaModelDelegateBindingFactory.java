...
public final class JavaModelDelegateBindingFactory {
	 ...
	 */
	public static final String JAVA_LANG_OBJECT_SIGNATURE = Signature
			.createTypeSignature("java.lang.Object", true); //$NON-NLS-1$
	 ...
	 */
	public ClassBinding getBindingForName(final String sig,
			final ClassFileParser visitor, final boolean removeGenerics) {
		String readableSignature = null;
		if (removeGenerics) {
			readableSignature = Signature.toString(Signature
					.getTypeErasure(sig));
		} else {
		...
		try {
			...
			case Signature.CLASS_TYPE_SIGNATURE:
				if (Signature.getTypeArguments(sig).length > 0
						&& !removeGenerics) {
					return getBindingForParameterizedType(sig, visitor,
							removeGenerics);
				}
			...
			case Signature.TYPE_VARIABLE_SIGNATURE:
				ITypeParameter typeParameter = visitor
						.findTypeParameter(readableSignature);
				return getTypeParameterBinding(typeParameter, visitor,
						removeGenerics);

			case Signature.CAPTURE_TYPE_SIGNATURE:
				// fall back to java.lang.Object
				return getBindingForName(
						JavaModelDelegateBindingFactory.JAVA_LANG_OBJECT_SIGNATURE,
	 ...
	 */
	public Binding getBindingForElement(final IJavaElement element,
		...
		try {
			...
			IStatus status = new Status(IStatus.WARNING,
					JavaActivator.PLUGIN_ID, e.getException().getMessage(), e);
	...
	public ClassBinding getBindingForWildCardType(final String signature,
		...
		if (boundSignature != null) {
			ClassBinding typeErasureBinding = getBindingForName(
					boundSignature, visitor, false);
	...
	public ClassBinding getBindingForParameterizedType(final String signature,
		...
		ClassBinding typeErasureBinding = getBindingForName(
				typeErasureSignature, visitor, removeGenerics);
		buffer.append(typeErasureBinding.toString());
		...
		if (!removeGenerics) {
			...
			// type arguments
			String[] typeArgumentsSignatures = Signature
					.getTypeArguments(signature);
			for (int i = 0; i < typeArgumentsSignatures.length; i++) {
				if (i > 0) {
					buffer.append(","); //$NON-NLS-1$
				}
				ClassBinding typeArgumentBinding = getBindingForName(
						typeArgumentsSignatures[i], visitor, removeGenerics);
	 ...
	 */
	public ClassBinding getBindingForPrimitiveType(final String signature,
			final ClassFileParser visitor) {
		ClassBinding binding = new ClassBinding();
	 ...
	 */
	public ClassBinding getBindingForArrayType(final String type,
			final ClassFileParser visitor, final boolean removeGenerics) {
		ClassBinding result = new ClassBinding();

		ClassBinding elementTypeBinding = getBindingForName(Signature
				.getElementType(type), visitor, removeGenerics);
		...
		result
				.setName(elementTypeBinding.toString()
						+ String.valueOf(brackets));
		return result;
	}

	private Binding getBinding(final IJavaElement element,
			final ClassFileParser visitor) throws JavaModelException {
		Binding result = null;
		if (element instanceof IMethod) {
			...
			result = getTypeParameterBinding((ITypeParameter) element,
					visitor, false);
	...
	private ClassBinding getTypeParameterBinding(final ITypeParameter element,
		...
		if (removeGenerics) {
			...
			return getBindingForName(Signature.createTypeSignature(
					erasure, true), visitor, removeGenerics);
		}
		ClassBinding result = new ClassBinding();
	...
	private MethodBinding getMethodBinding(final IMethod method,
			final ClassFileParser visitor) throws JavaModelException {
		MethodBinding result = new MethodBinding();

		result.setName(method.getElementName());
		result.setDeclaringClass(getClassBinding(
				method.getDeclaringType(), visitor));
		...
		for (int i = 0; i < method.getParameterTypes().length; i++) {
			result.getParameters().add(
					getParameterBinding(method.getParameterTypes()[i],
							visitor));
		}
		return result;
	}

	private ParameterBinding getParameterBinding(final String type,
			final ClassFileParser visitor) {
		...
		if (dimensions > 0) {
			result.setElementType(getBindingForName(Signature
					.getElementType(type), visitor, true));
		} else {
	...
	private PackageBinding getPackageBinding(final IPackageFragment pck,
			final ClassFileParser visitor) {
	...
	private ClassBinding getClassBinding(final IType type,
			final ClassFileParser visitor) throws JavaModelException {
		ClassBinding result = new ClassBinding();
		...
		if (type.getPackageFragment() != null
				&& type.getPackageFragment().exists()) {
			result.setOwnerPackage(getPackageBinding(type
					.getPackageFragment(), visitor));
		}
		if (type.getDeclaringType() != null && type.getDeclaringType().exists()) {
			result.setDeclaringClass(getClassBinding(type
					.getDeclaringType(), visitor));
		}
		if (type.isClass()) {
			...
			if (superClassSignature != null
				...
				result.setSuperClass(getBindingForName(
						superClassSignature, visitor, true));
			}
		}
		...
		if (!type.isAnnotation()
				&& type.getSuperInterfaceTypeSignatures() != null) {
			for (String superInterfaceSignature : type
					.getSuperInterfaceTypeSignatures()) {
				if (!superInterfaceSignature
						.equals(JavaModelDelegateBindingFactory.JAVA_LANG_OBJECT_SIGNATURE)) {
					result.getSuperInterfaces().add(
							getBindingForName(superInterfaceSignature,
									visitor, true));
				}
			}
		}
	...
	private FieldBinding getFieldBinding(final IField field,
			final ClassFileParser visitor) throws JavaModelException {
		...
		if (field.getDeclaringType() != null
				&& field.getDeclaringType().exists()) {
			result.setDeclaringClass(getClassBinding(field
					.getDeclaringType(), visitor));
		}
		return result;
	}
