...
import org.eclipse.gmt.modisco.java.io.java.binding.BijectiveMap;
...
/**
	...
	private void unSetProxy(final NamedElement element) {
		element.setProxy(false);
	...
	private Package createPackageHierarchy(final org.eclipse.jdt.core.dom.PackageDeclaration node) {
		...
		if (lastDotIndex == -1) {
			...
			while (lastDotIndex > 0) {
				...
				Package aParentPackage = (Package) getGlobalBindings().getTarget(
						currentPackageName);
				if (aParentPackage == null) {
					...
					aParentPackage = (Package) getGlobalBindings().getTarget(
							currentPackageName); // (PackageDeclaration)binding.get(node);
					aParentPackage.getOwnedPackages().add(localCurrentPackage);
	 ...
	 */
	private boolean isArrayLengthAccess(final org.eclipse.jdt.core.dom.QualifiedName node) {
	 ...
	 */
	private boolean isArrayLengthAccess(final org.eclipse.jdt.core.dom.SimpleName node) {
	 ...
	 */
	private boolean isArray(final org.eclipse.jdt.core.dom.Expression node) {
		if (node != null) {
	 ...
	 */
	private boolean isConstructorDeclaration(final org.eclipse.jdt.core.dom.SimpleName node) {
	...
	private boolean isQualifierChildAsName(final org.eclipse.jdt.core.dom.SimpleName node) {
		boolean result = false;
	 ...
	 *         {@code false} otherwise
	 */
	private boolean isNameChildOfMemberRef(final org.eclipse.jdt.core.dom.SimpleName node) {
	...
	public void endVisit(final org.eclipse.jdt.core.dom.SimpleName node) {
		if (this.isArrayLengthAccess(node)) {
			return;
		...
		if (node.isPackageMemberTypeDeclaration()) {
			...
			if (this.currentPackage == null) { // Type without package
				...
				if (this.currentPackage == null) {
					...
					getGlobalBindings().addTarget(JDTVisitor.DEFAULT_PKG_ID,
							this.currentPackage);
				}
			}
	 ...
	 */
	private void manageModifier(final org.eclipse.jdt.core.dom.Modifier jdtModifier,
