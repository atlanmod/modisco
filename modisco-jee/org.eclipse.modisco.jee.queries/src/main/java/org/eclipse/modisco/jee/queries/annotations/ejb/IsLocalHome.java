...
public class IsLocalHome implements IJavaModelQuery<ClassDeclaration, Boolean> {
	...
	static final String ANNOTATION_NAME = "LocalHome"; //$NON-NLS-1$
	static final String ANNOTATION_CLASS_NAME = "EJBLocalHome"; //$NON-NLS-1$
	...
	public Boolean evaluate(final ClassDeclaration context,
		...
		if (EvaluateAnnotation.evaluate(context,
				IsLocalHome.ANNOTATION_NAME).equals(
				Boolean.TRUE)) {
			return Boolean.TRUE;
		}
		return new ResolveInheritance().evaluate(context,
				IsLocalHome.ANNOTATION_CLASS_NAME);
	}
