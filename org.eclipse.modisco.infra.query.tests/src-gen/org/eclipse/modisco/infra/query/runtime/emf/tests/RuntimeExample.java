...
public class RuntimeExample {
	...
	public static void main(String[] args) {
		...
		//
		if (args.length == 0) {
			System.out.println(Messages.RuntimeExample_0);
			try {
				Resource resource = resourceSet.createResource(URI.createURI("http:///My.runtime")); //$NON-NLS-1$
				ModelQueryContext root = RuntimeFactory.eINSTANCE.createModelQueryContext();
			...
			for (int i = 0; i < args.length; ++i) {
				...
				try {
					...
					System.out.println(Messages.RuntimeExample_2 + uri);
					...
					System.out.println(Messages.RuntimeExample_4 + uri);
					exception.printStackTrace();
				}
			}
		}
