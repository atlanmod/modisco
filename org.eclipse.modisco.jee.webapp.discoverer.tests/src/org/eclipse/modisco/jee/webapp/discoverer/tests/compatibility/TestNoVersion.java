	...
	public void initResource() {
		if (this.resource == null) {
			...
			if (xmlFile.exists()) {
				...
				Assert.assertTrue(((WebXmlDiscoverer) xmlDiscoverer).getDescXmlVersion(JUnitPlugin.getDefault(), xmlFile, WebXmlDiscoverer.getRootName(),
						WebXmlDiscoverer.getDtdUrl())
						.equalsIgnoreCase(TestNoVersion.VERSION));

				this.resource = (Resource) parameters
						.get(DeploymentDescriptorDiscoverer.PARAMETER_TARGET_RESOURCE);

				Assert.assertNotNull(
						"Could not retrieve a xml model from "
								+ path.toString(), this.resource);
			} else {
				Assert.fail("Count not access the test file "
						+ path.toString());
			}
		}
	}
