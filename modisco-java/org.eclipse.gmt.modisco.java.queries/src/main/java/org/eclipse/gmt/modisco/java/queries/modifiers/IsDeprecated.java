...
public class IsDeprecated implements IJavaModelQuery<BodyDeclaration, Boolean> {
			...
			if (context.getAnnotations() != null) {
				for (Annotation annotation : context.getAnnotations()) {
					if (annotation != null && annotation.getType() != null
						...
						if ("Deprecated".equalsIgnoreCase(annotation.getType().getType().getName())) { //$NON-NLS-1$
							return true;
						}
					}
				}
